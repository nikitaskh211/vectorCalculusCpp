...
void test_div() {
  ...
  // Create a grid
  grid<unsigned int, float> g(3, 3, 3, 2.0f, 2.0f, 2.0f);

  // Create a vector field
  vectorField<unsigned int, float> vf(g);
  vf = [](float x, float y, float z) -> vec3d<float> {
    return vec3d<float>(complex<float>(x, 0), complex<float>(y, 0),
                        complex<float>(z, 0));
  };

  // Compute divergence
  scalarField<unsigned int, float> divResult = div<unsigned int, float>(vf);
...
void test_grad() {
  ...
  scalarField<unsigned int, float> sf(g);
  sf = [](float x, float y, float z) -> complex<float> {
    return complex<float>(x + y + z, 0);
  };

  // Compute gradient
  vectorField<unsigned int, float> gradResult = grad<unsigned int, float>(sf);

  // Verify results
  for (unsigned i = 0; i < g.nX; ++i) {
    for (unsigned j = 0; j < g.nY; ++j) {
      for (unsigned k = 0; k < g.nZ; ++k) {
        vec3d<float> expectedGrad(complex<float>(1, 0), complex<float>(1, 0),
                                  complex<float>(1, 0));
...
void test_curl() {
  ...
  vectorField<unsigned int, float> vf(g);
  vf = [](float x, float y, float z) -> vec3d<float> {
    return vec3d<float>(complex<float>(y, 0), complex<float>(z, 0),
                        complex<float>(x, 0));
  };

  // Compute curl
  vectorField<unsigned int, float> curlResult = curl<unsigned int, float>(vf);

  // Verify results
  for (unsigned i = 0; i < g.nX; ++i) {
    for (unsigned j = 0; j < g.nY; ++j) {
      for (unsigned k = 0; k < g.nZ; ++k) {
        vec3d<float> expectedCurl(complex<float>(1, 0), complex<float>(1, 0),
                                complex<float>(1, 0));
...
void test_laplace_scalar() {
  ...
  // Compute Laplacian
  scalarField<unsigned int, float> laplaceResult = laplaceScalar<unsigned int, float>(sf);
...
void test_laplace_vector() {
  ...
  vectorField<unsigned int, float> vf(g);
  vf = [](float x, float y, float z) -> vec3d<float> {
    return vec3d<float>(complex<float>(x * x, 0), complex<float>(y * y, 0),
                      complex<float>(z * z, 0));
  };

  // Compute Laplacian
  vectorField<unsigned int, float> laplaceResult = laplaceVector<unsigned int, float>(vf);

  // Verify results
  for (unsigned i = 0; i < g.nX; ++i) {
    for (unsigned j = 0; j < g.nY; ++j) {
      for (unsigned k = 0; k < g.nZ; ++k) {
        vec3d<float> expectedLaplace(complex<float>(2, 0), complex<float>(2, 0),
                                   complex<float>(2, 0));
